...
-싸이월드 > 게시판 > 년도별로
select * from 모든게시판;  > 테이블의 규모가 너무 크다 > 속도가 느려진다.
...
insert into tblUnionA values ('강아지');    -- 중복
insert into tblUnionA values ('고양이');    -- 중복
...
insert into tblUnionB values ('강아지');    -- 중복
insert into tblUnionB values ('고양이');    -- 중복
...
select * from tblUnionA
minus
select * from tblUnionB; -- 거북이,병아리,토끼

select * from tblUnionB
minus
select * from tblUnionA; -- 사자,코끼리,호랑이
