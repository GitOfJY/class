...
/*
    ...
    - 기존의 ANSI-SQL에 절차지향 언어의 기능을 추가한 SQL
...
-- dbms_output.put_line 의 결과 > 안보이게 설정 > set serverout on 보이게 직접 실행
set serverout on;
set serverout off;
-- 접속할 때 마다 다시 실행
...
declare
    num number; -- num 이름으로 number type 변수 선언
    name varchar2(20);
    today date;
begin
    num := 10;
...
-- 코드 조각 등록 > 도구 > 환경설정 > 코드 편집기 > 코드 템플릿 > syso, dbms_output.put_line(); 등록


declare
    num1 number;
    num2 number;
    num3 number := 30; -- 선언부에서 바로 초기화 가능
...
begin
    ...
    dbms_output.put_line(num2); -- 초기화 없이 출력 > null
    
    dbms_output.put_line(num3);
    
    --num4 number; > 구현부에서 변수 선언 불가능
...
--PLS-0020: identifier 'BUSEO' must be declared
begin
...
begin
    ...
    -- 2. 컬럼의 순서와 변수의 순서 일치
    --select name, buseo, jikwi into vname, vjikwi, vbuseo from tblInsa where num = 1001;
    -- 3. 컬럼과 변수의 자료형 일치
...
/*
    ...
    1. %type
        - 컬럼 1개 참조
        - 사용하는 테이블의 특징 컬럼의 스키마를 알아내서 변수에 적용
        - 복사되는 정보
            a. 자료형
            b. 길이
...
declare
    ...
    -- tblInsa의 buseo 칼럼을 확인해서 동일한 타입과 동일한 길이를 변수에 적용해준다.
begin
    select buseo into vbuseo from tblInsa where num = 1001;
end;
...
select * from tblInsa where city = '서울' and jikwi = '부장' and buseo = '영업부'; -- 김인수
...
declare
    ...
    -- vrow > 일종의 배열 역할 (PL/SQL에는 배열 존재하지 않는다.) > 레코드 역할
begin
    ...
    --select 
        ...
        --PL/SQL: ORA-00932: inconsistent datatypes: expected NUMBER got DATE (순서)
    ...
    --dbms_output.put_line(vrow); > 한번에 가져올 수 없음
...
--2. 서브쿼리
select seq from tblMember where name = '신숙주';
select video from tblRent where member = (select seq from tblMember where name = '신숙주');
...
/*
insert into tblBonus values (번호, vnum, vbasicpay*2);

1. 이미 seq 있을 때 > bonus.nextVal
create sequence seqBonus start with 4; 

2. seq 객체 사용하지 않을 때 > vseq
vseq number;
select max(seq) into vseq from tblBonus;
vseq := vseq+1;
*/
...
begin
    --open vcursor;
    --loop
    for vrow in vcursor loop -- loop +fetch into + vrwo + exit when
...
begin
    ...
    select name into vnmae from tblInsa where num = 1001;
    dbms_output.put_line(vnmae);
...
begin
    ...
    -- insert or update or delete 중 무슨일?
...
-- 특정 테이블에 여러개의 트리거 적용 > 관리 어려움 > 확인이 귀찮음 > 트리거끼리 충돌 or 재귀 호출
-- 공부할 때 > trigger 다 지우기 > 가만히 있어도 자원 소비
...
-- delete 1회 실행 > 적용된 행 9개 > 프로시저 1회 실행 > 문장 단위 프로시저
delete from tblMan; --9개
-- 레코드를 삭제했습니다.
-- 9개 행 이(가) 삭제되었습니다. 
...
-- delete 1회 실행 > 적용된 행 9개 > 프로시저 9번 실행 > 행 단위 프로시저
delete from tblMan;
...
-- 1.2 포인트 누적하기 (논리적 연관은 있지만 물리적 연관은 없음)
...
-- Case 2. 프로시저 > 90% 이상 (호출과제어를 마음대로 가능)
...
-- Case 3. 트리거 > 10% 사용 (24시간 항상 실행 중이라 사용 잘 안함)
