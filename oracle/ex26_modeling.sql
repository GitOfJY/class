...
/*
    ...
    정규화, Normalization
    - 자료의 손실이나, 불필요한 정보를 없애고, 데이어틔 일관성을 유지하며,
      데이터 종속서을 최소화하기 위해 ERD을 수정하는 작업
    ...
    1. 삽입 이상, Insertion Anormaly
        - 특정 테이블에 데이터를 삽입할 때 원하는 데이터까지 같이 넣어햐 하는 상황
    
    2. 갱신 이상, Update Anormaly
        - 동일한 데이터가 2개 시아의 테이블에 동시 저장 
        > 그 중 1개는 수정했지만 다른 1개를 수정하지 못 했을 때 상황
        > 두 데이터간의 이상 오류 발생
        > 즉, 2개의 데이터 중 어떤 데이터가 올바른 데이터인지 구분 불가능
    
    3. 삭제 이상, Deletion Anormaly
        - 특정 테이블의 데이터를 삭제할 때 원하지 않는 데이터까지 같이 지워지는 현상
        
        
    함수 종속, Fuctional Dependency
