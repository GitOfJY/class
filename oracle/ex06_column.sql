...
select
    first,
    nick,
    '코미디언',         -- 원본에 존재하지 않아도 가상의 컬럼 만들 수 있다.
    100,
    sysdate
from tblComedian;
...
-- 14개 국가가 속한 대륙의 이름을 가져오시오.
select continent from tblCountry;

-- tblCountry에는 어떤 대륙이 있습니까? > 종류, 그룹 (중복 없이)
...
--중복값이 존재하지 않을 때 distinct > 아무일도 일어나지 않는다.
select distinct name from tblCountry;


select*from tblCountry;
select distinct continent from tblCountry;
--각 대륙에 어떤 나라가 있는지 국가명도 함께 출력하시오.
...
select 
    ...
    when continent = 'NA' then '북아메리카'
     ...
     -- else '기타'
     -- else name  > 다른 컬럼 출력 가능하지만 사용 금지!!
     -- 하나의 속성(컬럼)안에는 같은 도메인(성격,표현)을 가지는 값이 들어간다.
     -- else population > 자료형 때문에 오류
     -- (***) 모든 반환값이 동일한 성격 + 동일한 자료형을 가져야 한다.
     else continent -- null 대신 원본값 그대로 돌려준다.
...
--직위별 수당 지급 > 차등 지급 > 부장(x2),과장(x1.7),대리(x1.5),사원(x1.2)
--직위별 수당 지급 > 차등 지급 > 부장,과장(x2),대리,사원(x1.5)
