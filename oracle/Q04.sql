...
select sum(salary) from hr.employees where manager_id='108';
...
select sum(basicpay + sudang) from tblInsa where jikwi like '%장';
...
select
    avg(case when jikwi like '%장' then basicpay end) - 
    avg(case when jikwi in ('대리', '사원') then basicpay end)
    as "평균 급여 차이"
from tblInsa; 
