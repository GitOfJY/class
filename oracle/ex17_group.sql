...
select round(avg(basicpay)) from tblInsa; -- 60명 기준 155만원
...
select 
    ...
    count(decode(gender, 'f', 1)) as "남자수"
from tblComedian;
...
select
   count(
    case
         when completedate is not null then '0'
    end),
    count(
    case
         when completedate is null then '0'
    end)
from tblTodo;
...
-- pivot : 가로/세로 방향 바꿀 수 있음

-- 새로운 데이터/ 수정된 데이터에 아주 강함 > 정적/동적 데이터 모두 무난
select
    city,
    count(*)
from tblInsa 
...
select                                      -- 4 > 평균 급여 < 사전 조건(급여 150만원 이상 사람들)
    buseo,
    round(avg(basicpay))
...
select                                                    -- 4 
    ...
    round(avg(basicpay))
from tblInsa                                           -- 1 > 60명의 데이터를 가져온다.
    group by buseo                                  -- 2 > 60명을 대상으로 그룹 형성한다.
...
from tblInsa                                            -- 1 > 60명 데이터를 가져온다.
    where basicpay >=1500000                   -- 2 > 27명만 남긴다. (개인 탈락)
        ...
        having avg(basicpay) >= 2200000      -- 4 > 4개 부서만 남긴다. (부서 탈락)
            order by 2;                                 -- 6 > 정렬



-- 서울 직원 > 남/녀 그룹 > 그룹별 평균 급여 155만원 이상 > 각 그룹의 인원 수?
select
   substr(ssn, 8, 1),
   count(*)                                               -- 5
from tblInsa                                             -- 1
    ...
    group by substr(ssn, 8, 1)                       -- 3
        having avg(basicpay) >= 155000;        -- 4



-- tblCountry
-- 대륙별로 최대 인구수, 최소 인구수, 평균 인구수
select
    continent as "대륙", 
...
select
    buseo, jikwi,
    count(*)
from tblInsa group by buseo, jikwi;
...
/*
    ...
    rollup()
    - group by의 결과 집계 결과를 좀 더 자세하게 반환
