...
select name, buseo, rownum from tblInsa where rownum = 3; -- 2 ~ 60 검색 안됨
select name, buseo, rownum from tblInsa where rownum > 5; -- 검색 안됨


select name, buseo, rownum     -- 2. 소비 > 1에서 이미 할당된 숫자를 rownum 표현을 통해 가져온다. (여기서 생성 아님)
...
select 
    name, buseo, basicpay, 
    rownum,                                -- B절 실행될 때 할당된 번호 
    rnum
from (select                               -- B절
            name, buseo, basicpay, 
            rownum as rnum                 -- A절 실행될 때 할당된 번호
        from tblInsa                       -- A절
...
create or replace view vwAdressBook
as
select a.*, rownum as rnum from (select * from tblAddressBook order by name asc) a;


select * from vwAdressBook where rnum between 31 and 40;
