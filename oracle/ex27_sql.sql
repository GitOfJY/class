...
-- 계층형 쿼리 > 오라클에서 자주 사용, 연습하기
-- select * from tblSelf start with 루트노드조건 connect by 조건;
-- (start with 루트노드조건) 생략가능
select 
    level,                        -- 깊이
    prior name as "상사이름",     -- 부모이름
    name as "직원이름"            -- 자식(나) 이름
from tblSelf
...
select 
    level 
from dual 
    connect by level <= 31;    -- level이 곧 루프와 같은 역할

select 
    sysdate + (level-1)        -- 날짜(일) 연산
from dual 
    connect by level <= 31;
...
-- 이 쿼리 잘 기억해두기 > date 자료형으로 원하는 기간의 레코드를 얻어오는 방법
select 
    to_date('2022-03-01', 'yyyy-mm-dd') + (level - 1) as regdate
from dual
...
select regdate from vwDate;     -- 일련의 날짜
select * from tblDate;          -- 근태 기록

select 
    * 
from vwDate v                             -- 부모 (일부만 나옴)
    inner join tblDate t                  -- 자식 (전부 나옴)
        on v.regdate = t.regdate;         -- 근태기록이 없는 날짜도 필요 > inner join 사용 아님
...
-- midx number not null, --외래키로 지정할 컬럼 
