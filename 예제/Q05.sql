...
select 
    first_name || last_name as "fullname",
    length(first_name)+length(last_name) as "namelength"
from employees 
    order by length(first_name)+length(last_name) asc;
...
select 
    max(length(first_name)+length(last_name)) as "가장 긴 사람",
    min(length(first_name)+length(last_name)) as "가장 짧은 사람",
   round( sum(length(first_name)+length(last_name)) / count(*)) as "평균"
from employees ;
...
from tblInsa;
-- 1855714	1601513	1714857	1387857
...
from tblInsa;
-- 13341657  5836975	13341657	6670829
-- sum basicpay 부분에서 부서별 basicpay가 아님
