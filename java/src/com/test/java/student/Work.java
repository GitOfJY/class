...
public class Work {
	...
	private void delet() {
		...
		for (Student s : Data.slist) {
			if (s.getSeq().equals(input)) {
				// Data.slist.remove(s); 컬렉션에서 삭제하면 끝
				result = s;
				break;
			}
		}
	...
	private void edit() {
		...
		for (Student s : Data.slist) {
			if (s.getSeq().equals(input)) {
				result = s; // 주소값 복사 > result 수정하면 s도 수정 > arraylist도 수정 >> 같은 주소값
							// 참조형 장점 > 편리!
				break;
			...
			for (Score s : Data.clist) {
				if (s.getSeq().equals(result.getSeq())) {
					System.out.printf("국어(%d), 영어(%d), 수학(%d) = 평균(%.1f)\n", s.getKor(), s.getEng(),
							s.getMath(), (s.getKor() + s.getEng() + s.getMath()) / 3.0);
				}
			}
