...
public class FileController {
	...
	//public String addok(String data) { > 텍스트 가져오기
	public String addok(String data, HttpServletRequest req, Model model) { //HttpServletRequest > 파일 업로드시 필요
		...
		//<form> 파일 > MultipartHttpServletRequest 전달 > multipart와 비슷
		...
		//System.out.println(file.getName()); 	     		//attach
		//System.out.println(file.getOriginalFilename());   //0.png
		//System.out.println(file.getContentType());		//image/png
		//System.out.println(file.getSize()); 			    //14029
		
		//파일 업로드 완료 > 파일이 어디에 있는지? > 스프링이 관리하는 임시 폴더에 저장 > 우리가 원하는 곳으로 이동
		...
		//파일 이동하기
		File file2 = new File(path + "\\" +filename); //path > 업로드할 폴더, filename > 업로드할 파일 >> 희망경로
